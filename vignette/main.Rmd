...
acc <- group_by(acc_by_subject,
                group, test, practice, practice_cue_type) %>%
  summarize(mean_percent_correct = mean(percent_correct),
            sem_percent_correct = whoppeR::sem(percent_correct)) %>%
  rename(percent_correct = mean_percent_correct) %>%
  ungroup()
```

```{r plot_group_averages}
acc_plot <- mutate(acc,
                   condition = as.character(interaction(practice, practice_cue_type))
                   ) %>%
  replace_na(list(condition = "N")) %>%
  filter(test == 2) %>%
