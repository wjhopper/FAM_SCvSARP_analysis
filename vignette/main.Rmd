...
select(as.data.frame(summary(acc_glmer_contrasts)), group, contrast, estimate:p.value) %>%
knitr::kable(format = 'html', align = 'crccccc', digits = c(0,0,3,3,3,3,4),
             caption = paste(attr(acc_glmer_contrasts,"mesg"), collapse = ". ")) %>%
...
```{r rstan_acc_glmer_uninformed_contrasts}
...
#### Informed priors
```{r rstan_acc_glmer_informed-priors}
...
```{r rstan_acc_glmer_informed-priors_contrasts}
...
select(as.data.frame(summary(RT_lmer_contrasts)), group, contrast, estimate:p.value) %>%
knitr::kable(format = 'html', align = 'crccccc', digits = c(0,0,3,3,3,3,4),
             caption = paste(attr(RT_lmer_contrasts,"mesg"), collapse = ". ")) %>%
  kableExtra::kable_styling(full_width = FALSE) %>%
  kableExtra::collapse_rows(columns = 1)
```
...
### rstanarm::stan_lmer
```{r RT_rstan_glmer}
RT_rstan_lmer_uninformed <- stan_lmer(logRT ~ group*condition + (condition|subject),
                                      data = RT_lmer_data,
                                      contrasts = list(group = 'contr.sum',
                                                       condition = 'contr.sum'),
                                      seed = 123, adapt_delta = .975)

summary(RT_rstan_lmer_uninformed,
        pars = names(fixef(RT_rstan_lmer_uninformed)))
```

```{r RT_rstan_glmer_contrasts}
